...
function getAnimationFrame (
  ...
  if (isSpacieUpBMovementAction || isDamageFlyRoll) {
    ...
    const referenceFrame = isSpacieUpBMovementAction
      ...
      : playerFrame.isNana
        ...
        : frames[playerFrame.frameNumber - 1].players[playerFrame.playerIndex]
          .state
    const deltaFrame = playerFrame.isNana
...
function drawHexagon (radius: number, worldContext: CanvasRenderingContext2D) {
  worldContext.beginPath()
  const sixths = (2 * Math.PI) / 6
  worldContext.moveTo(0, radius)
  for (let hexPart = 0; hexPart < 6; hexPart++) {
    worldContext.lineTo(
