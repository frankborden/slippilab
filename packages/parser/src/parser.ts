...
/** */
export function parseReplay(fileBuffer: ArrayBuffer): Replay {
...
function initFrameIfNeeded(frames: Frame[], frameNumber: number) {
  if (!frames[frameNumber]) {
    // @ts-ignore: randomSeed will be populated later if found.
    frames[frameNumber] = {
...
function initPlayerIfNeeded(
  ...
  if (!frames[frameNumber].players[playerIndex]) {
    // @ts-ignore: state and inputs will be populated later.
...
function parsePostFrameUpdateEvent(
  ...
  return {
    ...
    isGrounded: !Boolean(
      readUint(rawData, 8, replayVersion, '2.0.0.0', offset + 0x2f),
    ),
