import { actions, attacks, ReplayData } from "@slippilab/common";
...
export function isOffstage(
   ...
   */
  const actionName = actions[state.actionStateId];
...
export function action(actionName: string): Predicate {
  return (playerIndex: number, frameNumber: number, replay: ReplayData) => {
    ...
    return actions[actionStateId] === actionName;
  };
}
...
export function actionStartsWith(actionName: string): Predicate {
  return (playerIndex: number, frameNumber: number, replay: ReplayData) => {
    ...
    return actions[actionStateId]?.startsWith(actionName);
  };
}

export function landsAttack(attackName: string): Predicate {
  return all(
