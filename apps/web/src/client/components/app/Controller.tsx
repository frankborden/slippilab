...
export function Controller(props: {
  ...
  return (
    <>
      <ControllerSVG replay={props.replay} renderData={props.renderData} />
      <Summary renderData={props.renderData} />
    </>
  );
}

function Summary(props: { renderData: RenderData }) {
  return (
    <div class="grid w-full grid-cols-[repeat(2,auto)] gap-x-4 text-sm">
      <div>Action:</div>
      <div>{props.renderData.animationName}</div>
      <div>Frame:</div>
      <div>
        {roundHundredth(props.renderData.playerState.actionStateFrameCounter)}
      </div>
      <Show when={props.renderData.playerState.hitlagRemaining > 0}>
        <div>Hitlag</div>
        <div>{props.renderData.playerState.hitlagRemaining}</div>
      </Show>
      <Show when={props.renderData.playerState.isInHitstun}>
        <div>Hitstun</div>
        <div>{props.renderData.playerState.hitstunRemaining}</div>
      </Show>
      <Show
        when={
          props.renderData.playerState.hurtboxCollisionState !== "vulnerable"
        }
      >
        <div>Status</div>
        <div class="capitalize">
          {props.renderData.playerState.hurtboxCollisionState}
        </div>
      </Show>
    </div>
  );
...
function ControllerSVG(
  ...
  const inputs = createMemo(() => props.renderData.playerInputs);
