...
function Battlefield() {
  ...
  return (
    <>
      ...
      <polyline points={mainStage.join(" ")} class={cn(stageFillColor)} />
      <For each={platforms}>
        {(points) => (
          <polyline points={points.join(" ")} class={cn(stageStrokeColor)} />
        )}
      </For>
      <rect
        x={blastzones[0][0]}
  ...
  return (
    <>
      ...
      <polyline
        ...
        class={cn(stageFillColor, stageStrokeColor)}
      />
      <For each={platforms}>
        {(points) => (
          <polyline points={points.join(" ")} class={cn(stageStrokeColor)} />
      ...
      <polyline points={randall().join(" ")} class={cn(randallStrokeColor)} />
      <rect
        x={blastzones[0][0]}
        y={blastzones[0][1]}
        width={blastzones[1][0] - blastzones[0][0]}
...
function Grid(props: { blastzones: number[][] }) {
  ...
  return (
    <For each={lines()}>
      {([x1, x2, y1, y2]) => (
        <line
          class={cn(gridStrokeColor, "stroke-[0.1]")}
          x1={x1}
          x2={x2}
          y1={y1}
          y2={y2}
