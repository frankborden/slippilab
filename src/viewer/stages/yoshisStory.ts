...
export const yoshisStory: Stage = {
  ...
  getMovingPlatforms: (frame: number) => {
    ...
    // must be LGPL/GPL as well.
    const frame_count = frame % 1200;
    const randall_width = 11.9;

    // Top section
    if (476 < frame_count && frame_count < 1016) {
      const start = 101.235443115234;
      const speed = -0.35484;
      const frames_in = frame_count - 477;
      const y = -13.64989;
      const left = new Vector(start - randall_width + speed * frames_in, y);
      const right = new Vector(start + speed * frames_in, y);
      return [[left, right]];
    }
    // Left section
    if (1022 < frame_count && frame_count < 1069) {
      const start = -15.2778692245483;
      const speed = -0.354839325;
      const frames_in = frame_count - 1023;
      const y = start + speed * frames_in;
      const left = new Vector(-103.6, y);
      const right = new Vector(-91.7, y);
      return [[left, right]];
    }
    // Bottom section
    if (frame_count > 1075 || frame_count < 416) {
      const start = -101.850006103516;
      const speed = 0.35484;
      const frames_in = frame_count + (frame_count < 416 ? 125 : -1076);
      const y = -33.2489;
      const left = new Vector(start + speed * frames_in, y);
      const right = new Vector(start + randall_width + speed * frames_in, y);
      return [[left, right]];
    }
    // Right section
    if (423 < frame_count && frame_count < 469) {
      const start = -31.160232543945312;
      const speed = 0.354839325;
      const frames_in = frame_count - 424;
      const y = start + speed * frames_in;
    ...
    const position = _RANDALL_CORNER_POSITIONS[frame_count];
    const y = position[0];
    const left = new Vector(position[1], y);
    const right = new Vector(position[1] + randall_width, y);
    return [[left, right]];
  },
};
